<!-- 以下方式定时转到其他页面 -->
<meta http-equiv="refresh" content="1;url=Huodong.php">
<?php
...
//   ./upload/  和 "upload/" 当前目录
    ...
    $path = "musicialActivityPicture";//定义一个上传后的目录
    if (!(file_exists($path))) {
    ...
    if (is_uploaded_file($tmp_name)) {//判断是否是一个上传的文件
        if (move_uploaded_file($tmp_name,$path)) {//执行文件上传(移动上传文件)
            ...
            $sql="insert into HDmain(pic,name,url,other)
       values('$filename','$name','$url','$other')";
            $res=$db->exec($sql);
            $db=null;
            echo $path;
            echo "<p><a href='Huodong.php'>等待返回活动页面，如果不能返回，请点击这里</a></p>";
